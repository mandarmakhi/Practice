...
#Here there are many variables that we need to convert into an Ctaegorical Vriable. 
...
x <- roc(salary_test$Salary,predictor = factor(pred, ordered = TRUE))
plot(x)
y <- area$auc
y
...
x1 <- roc(salary_test$Salary,predictor = factor(pred1, ordered = TRUE))
plot(x1)
